...
'''
Character to binary value mapping 7 segment display.
...
def toSegment(text, dp):
   data = []
   for i in range(4):
       data.append(PATTERN[text[i]] + dp[i] * 128)
   return data

def showText(text, dp):
    data = toSegment(text, dp)
...
showText("bo  ", [0, 1, 0, 0])
time.sleep(1)
showText("boo ", [0, 0, 1, 0])
time.sleep(1)
showText("boot", [0, 0, 0, 1])
time.sleep(1)
showText("    ", [1, 1, 1, 1])
